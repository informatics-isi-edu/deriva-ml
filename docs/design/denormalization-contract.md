# Denormalization — Implementation Contract (maintainer guide)

!!! warning "This is the internal design contract, not user-facing API docs."
    This page is for **engineers and auditors** working on
    `deriva_ml.local_db.denormalize`, `local_db.denormalizer`,
    `local_db.paged_fetcher`, and `model.denormalize_planner`. It pins
    *how the code is required to behave* — the state model, the fetcher /
    INSERT contracts, the row-completeness invariant, the fragility map,
    and the test matrix.

    **If you just want to *use* denormalization**, read the
    [Denormalization user guide](../user-guide/denormalization.md)
    (concepts + examples) and the
    [Denormalization reference](../reference/denormalization.md)
    (formal rules + method signatures) instead.

The six semantic rules (Row grain, Column projection, Column hoisting,
Downstream-leaf rejection, Path ambiguity, Anchor disposition) are stated
canonically in the [reference](../reference/denormalization.md#formal-rules);
§6.2 below restates them in full contract language with the planner-level
guarantees. If a rule's *semantics* change, update the reference first,
then this section.
...
4. **The six semantic rules are canonical in the
   [reference](../reference/denormalization.md#formal-rules).**
   Three documents describe them at different depths and must
   stay in lockstep when a rule's *semantics* change:
   the [reference](../reference/denormalization.md) (formal
   D-rules — the source of record), the
   [user guide](../user-guide/denormalization.md) (the
   example-led summary), and §6.2 of *this* doc (full contract
   language with planner-level guarantees). Update the reference
   first, then propagate to the other two. The `Denormalizer`
   class docstring lists the rule *names* and links to the
   reference; it does not restate them.
