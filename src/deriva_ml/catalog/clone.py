"""Catalog workspace creation with cross-server and selective asset support.

This module provides `create_ml_workspace` for creating a partial catalog clone
containing data reachable from a root RID. It handles the common case of
incoherent row-level policies in source catalogs.

The solution uses a three-stage approach:
1. Create schema WITHOUT foreign keys
2. Copy all data (using async operations for performance)
3. Apply foreign keys, handling violations by either:
   ...
   - Failing the operation (reporting violations)
"""
...
def get_catalog_provenance(catalog: ErmrestCatalog) -> CatalogProvenance | None:
    ...
    Returns provenance information if the catalog has it set.
...
def _rebuild_deferred_indexes(
    ...
    """Report on indexes that had issues during data copy."""
...
def _parse_export_annotation_tables(
    ...
    should be exported when downloading a row as a BDBag.

    Args:
        table: ERMrest Table object with annotations.
        paths_discovered: Optional list to append discovered paths to.
...
def _discover_reachable_tables(
    ...
    Traverses FK graph in both directions (outbound and inbound FKs).

    Args:
        model: ERMrest Model object.
        start_tables: Starting tables in "schema:table" format.
        exclude_tables: Set of (schema, table) tuples to exclude.
        exclude_schemas: Set of schema names to exclude.
...
def _expand_tables_with_associations(
    ...
    Returns:
        Tuple of (all_tables, association_tables_added).
    """
...
# =============================================================================
# Schema Building (extracted from _clone_three_stage)
# =============================================================================
...
def _build_subset_schema(
    ...
    report: CloneReport,
) -> tuple[list[dict], list[tuple[str, str, dict]], int]:
    """Build schema model JSON for subset of tables, deferring foreign keys.

    Args:
        src_model: Source catalog model.
        included_tables: Set of (schema, table) tuples to include.
        copy_annotations: Whether to copy annotations.
        copy_policy: Whether to copy ACL policies.
        prune_hidden_fkeys: Whether to prune FKs with hidden reference data.
        report: Clone report for recording issues.

    Returns:
        Tuple of (model_json, deferred_fkeys, fkeys_pruned) where:
        - model_json: List of schema/table definitions for POST /schema
        - deferred_fkeys: List of (schema, table, fk_def) tuples
        - fkeys_pruned: Number of FKs that were pruned
    """
    new_model = []
    fkeys_deferred = []
    fkeys_pruned = 0
    ...
    for sname in included_schemas:
        ...
        for tname, table in schema.tables.items():
            ...
            for fkdef in table.prejson().get('foreign_keys', []):
                ...
                # Check if FK references only included tables
                include_fk = True
                for ref_col in fkdef.get('referenced_columns', []):
                    ref_schema = ref_col.get('schema_name')
                    ref_table = ref_col.get('table_name')
    ...
    return new_model, fkeys_deferred, fkeys_pruned


# =============================================================================
# FK Application with Orphan Handling (extracted from _clone_three_stage)
# =============================================================================
...
def _apply_fkeys_with_orphan_handling(
    dst_catalog: ErmrestCatalog,
    fkeys_deferred: list[tuple[str, str, dict]],
    orphan_strategy: OrphanStrategy,
    report: CloneReport,
) -> tuple[int, int]:
    """Apply deferred foreign keys with orphan handling.

    Args:
        dst_catalog: Destination catalog.
        fkeys_deferred: List of (schema, table, fk_def) tuples.
        orphan_strategy: How to handle orphan rows.
        report: Clone report for recording issues.

    Returns:
        Tuple of (orphan_rows_removed, orphan_rows_nullified).
    """
    orphan_rows_removed = 0
    orphan_rows_nullified = 0

    if orphan_strategy == OrphanStrategy.DELETE:
        # Three-phase approach: identify orphans, delete, then apply FKs

        # Phase 1: Identify orphan values for each FK
        logger.info("Phase 1: Identifying orphan values...")
        fk_orphans: list[tuple[str, str, dict, set]] = []
        ...
        # Phase 2: Delete orphan rows in cascading waves
        if fk_orphans:
            logger.info("Phase 2: Deleting orphan rows...")

            max_waves = 20
            all_processed_fks: set[tuple[str, str, str]] = set()
        ...
        # For NULLIFY or FAIL strategies: single-pass approach
        for sname, tname, fk in fkeys_deferred:
            try:
                dst_catalog.post("/schema", json=[fk])
                report.fkeys_applied += 1
    ...
    return orphan_rows_removed, orphan_rows_nullified
...
def _post_clone_operations(
    ...
    if alias:
        try:
            server.create_ermrest_alias(id=alias, alias_target=str(result.catalog_id))
            result.alias = alias
        except Exception as e:
...
# =============================================================================
# Async Data Copy Helpers
# =============================================================================


async def _copy_path_query_async(
    ...
    """Copy data using an async path query with pagination.

    Fetches data from the source via a path query and inserts into the destination.
    Handles duplicate rows (via onconflict=skip) and btree index errors (via
    row-by-row fallback).

    Args:
        src_async: Source async catalog.
        dst_async: Destination async catalog.
        path_query: ERMrest path query (e.g., /entity/Schema:Table/RID=X/Schema:Next).
        table_spec: Target table in "schema:table" format.
    ...
    while True:
        ...
        try:
            response = await src_async.get_async(query)
            page = response.json()
        except Exception as e:
        ...
        if truncate_oversized:
            page = [_truncate_row_values(row) for row in page]

        # Insert into destination with onconflict=skip to handle overlapping paths
        dst_path = f"/entity/{_quote_table_spec(table_spec)}?nondefaults=RID,RCT,RCB&onconflict=skip"
        try:
            await dst_async.post_async(dst_path, json_data=page)
            ...
            if _BTREE_INDEX_ERROR_PATTERN in error_msg:
                ...
                for row in page:
                    try:
                        await dst_async.post_async(dst_path, json_data=[row])
                        rows_copied += 1
                    except Exception:
                        logger.debug(f"Skipping row due to index error")
            else:
                logger.warning(f"Failed to insert into {table_spec}: {e}")
...
def _truncate_row_values(row: dict) -> dict:
    """Truncate oversized text values in a row for btree index compatibility."""
    modified = row.copy()
    for col, value in row.items():
        if isinstance(value, str):
            value_bytes = len(value.encode('utf-8'))
            if value_bytes > _MAX_INDEX_VALUE_BYTES:
                max_chars = _MAX_INDEX_VALUE_BYTES - len(_TRUNCATE_SUFFIX.encode('utf-8'))
                truncated = value[:max_chars] + _TRUNCATE_SUFFIX
                while len(truncated.encode('utf-8')) > _MAX_INDEX_VALUE_BYTES:
                    max_chars -= 100
                    truncated = value[:max_chars] + _TRUNCATE_SUFFIX
...
async def _copy_data_via_export_paths_async(
    src_async: AsyncErmrestCatalog,
    dst_async: AsyncErmrestCatalog,
    root_table: str,
    root_rid: str,
    ...
    """Copy data by following export paths asynchronously.

    Copies the root row, then follows each export path to copy related data.
    Tables not covered by export paths (e.g., vocabulary tables) are noted
    as uncovered for later fill-in.
    ...
    # Copy root row
    root_query = f"/entity/{_quote_table_spec(root_table)}/RID={urlquote(root_rid)}"
    try:
        response = await src_async.get_async(root_query)
        root_row = response.json()
        if root_row:
            dst_path = f"/entity/{_quote_table_spec(root_table)}?nondefaults=RID,RCT,RCB"
            await dst_async.post_async(dst_path, json_data=root_row)
    ...
    for path in export_paths:
        ...
        rows = await _copy_path_query_async(
            src_async, dst_async, query, target_table,
            page_size=page_size, truncate_oversized=truncate_oversized,
        )
...
async def _create_ml_workspace_async(
    ...
    Creates async catalog connections, copies data via export paths,
    then fills in uncovered tables using concurrent async table copy.
    ...
    dst_async = AsyncErmrestCatalog(
        "https", dest_hostname, str(dst_catalog.catalog_id), dst_cred
    )
    src_wrapper = AsyncCatalogWrapper(src_catalog, src_async)

    try:
        rows_by_table: dict[str, int] = {}

        if export_paths:
            # Copy data via export paths (respects row-level ACLs)
            if progress_callback:
                progress_callback("Copying data via export paths", 30.0)

            rows_by_table = await _copy_data_via_export_paths_async(
                src_async, dst_async,
        ...
        # Fill in tables not covered by export paths
        # (vocabulary tables, association tables, etc.)
        fill_tables = [t for t in all_tables if t not in rows_by_table]
        if fill_tables:
            if progress_callback:
...
def create_ml_workspace(
    ...
    *,
    ...
    Args:
        ...
        include_tables: Optional additional tables ("schema:table" format)
            to include beyond what's discovered from the root table.
        exclude_objects: Tables ("schema:table" format) to exclude.
        exclude_schemas: Schema names to exclude entirely.
    ...
    if progress_callback:
        progress_callback("Connecting to source catalog", 0.0)
    ...
    logger.info(
        f"Connected to source catalog {source_hostname}/{source_catalog_id} "
    ...
    # Capture source schema for provenance
    source_schema_json = src_catalog.get("/schema").json()

    # Find the table containing the root RID
    if progress_callback:
    ...
    if discovered_tables and len(discovered_tables) > 1:
        ...
        # Fall back to FK graph traversal
        logger.info("No export annotation found, using FK graph traversal")
        export_paths = []
    ...
    logger.info(f"Will clone {len(all_tables)} tables")

    # Parse tables into set for schema building
    ...
    if progress_callback:
        progress_callback("Creating destination catalog", 20.0)
    ...
    dst_catalog_id = str(dst_catalog.catalog_id)
    ...
    try:
        ...
        # Build schema without FKs
        new_model, fkeys_deferred, fkeys_pruned = _build_subset_schema(
            src_model=src_model,
        ...
        # =====================================================================
        # Phase 3: Async Data Copy
        # =====================================================================

        if progress_callback:
            progress_callback("Copying data", 25.0)

        logger.info("Stage 2: Copying data asynchronously...")
        rows_by_table = asyncio.run(
            _create_ml_workspace_async(
        ...
        # Update report with copied rows
        total_rows_copied = 0
        for table_key, rows in rows_by_table.items():
            report.tables_restored[table_key] = rows
            total_rows_copied += rows

        logger.info(f"Stage 2 complete: {total_rows_copied} rows copied across {len(rows_by_table)} tables")

        # =====================================================================
        # Phase 4: FK Application
        # =====================================================================

        if progress_callback:
            progress_callback("Applying foreign keys", 70.0)

        logger.info(f"Stage 3: Applying {len(fkeys_deferred)} foreign keys...")
        orphan_rows_removed, orphan_rows_nullified = _apply_fkeys_with_orphan_handling(
            dst_catalog, fkeys_deferred, orphan_strategy, report,
        )
        report.fkeys_pruned = fkeys_pruned

        # Copy configuration (annotations, policies)
        if copy_annotations or copy_policy:
            _copy_configuration(
                src_model, dst_catalog, copy_annotations, copy_policy,
                exclude_schemas or [], excluded_tables,
            )

        # =====================================================================
        # Phase 5: Post-Clone Operations
        # =====================================================================

        if progress_callback:
            progress_callback("Post-clone operations", 85.0)
        ...
        # Upload source schema and set provenance
        ...
        provenance = CatalogProvenance(
            ...
            description=f"Partial clone from {source_hostname}:{source_catalog_id} (root: {root_rid})",
            clone_details=clone_details,
        )
        _set_catalog_provenance(dst_catalog, provenance)

        # Post-clone operations (alias, ML schema)
        ...
        if reinitialize_dataset_versions:
            result = _reinitialize_dataset_versions(result, dst_cred)

        # Set defaultTable to the root table
        try:
        ...
        logger.info(
            f"Workspace creation complete: {dest_hostname}/{dst_catalog_id} "
            f"({total_rows_copied} rows in {len(rows_by_table)} tables)"
        )
