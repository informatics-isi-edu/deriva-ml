...
def table_paths(graph: dict[Table, list[dict[Table, Any]]], path: Optional[list[Table]] = None) -> list[list[Table]]:
...
def nested_dataset_schema_graph(model: Model)  -> dict[Table, list[dict[Table, list]]]:
    dataset_table = model.schemas['deriva-ml'].tables["Dataset"]
    return {model.schemas['deriva-ml'].tables['Dataset_Dataset']: [schema_graph(model, dataset_table)]}
...
def schema_graph(model: Model,
    ...
    for t in nodes:
        ...
        graph[node].append(schema_graph(model, t, new_visited_nodes))
    return graph
...
def dataset_specification(model: Model,
    ...
    element_spec = []
    for path in table_paths(nested_dataset_schema_graph(model), [dataset_table]):
