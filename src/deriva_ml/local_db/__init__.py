...
``docs/user-guide/denormalization.md`` for the current architecture,
state model, fetcher/INSERT contract, fragility map, the nine
semantic Rules, and the test matrix. The older Phase 1 / Phase 2
design specs under ``docs/superpowers/specs/`` remain authoritative
for the workspace layout (ATTACH'd schema files, ``main.db``
registries), but are superseded by the user-guide doc wherever they
overlap on fetcher/INSERT-side behavior or denormalize semantics.
