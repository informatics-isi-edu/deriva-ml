...
class TestBagDenormalizeColumns:
    ...
    def test_denormalize_columns_returns_tuples(self, catalog_manager: CatalogManager, tmp_path: Path):
    ...
    def test_denormalize_columns_matches_dataframe(self, catalog_manager: CatalogManager, tmp_path: Path):
        ...
        assert set(col_names) == set(df_cols), (
            f"Column mismatch: denormalize_columns={sorted(col_names)}, dataframe={sorted(df_cols)}"
        )

    def test_denormalize_columns_includes_types(self, catalog_manager: CatalogManager, tmp_path: Path):
...
class TestBagListDatasetElementTypes:
    ...
    def test_element_types_non_empty(self, catalog_manager: CatalogManager, tmp_path: Path):
    ...
    def test_element_types_match_member_tables(self, catalog_manager: CatalogManager, tmp_path: Path):
...
class TestBagListExecutions:
    ...
    def test_list_executions_returns_list(self, catalog_manager: CatalogManager, tmp_path: Path):
...
class TestBagListTables:
    ...
    def test_list_tables_returns_qualified_names(self, catalog_manager: CatalogManager, tmp_path: Path):
    ...
    def test_list_tables_includes_domain_tables(self, catalog_manager: CatalogManager, tmp_path: Path):
...
class TestBagTableAccess:
    ...
    def test_get_table_as_dict_yields_dicts(self, catalog_manager: CatalogManager, tmp_path: Path):
    ...
    def test_get_table_as_dataframe_returns_df(self, catalog_manager: CatalogManager, tmp_path: Path):
    ...
    def test_get_table_as_dataframe_matches_dict(self, catalog_manager: CatalogManager, tmp_path: Path):
...
class TestBagCurrentVersion:
    ...
    def test_current_version_returns_dataset_version(self, catalog_manager: CatalogManager, tmp_path: Path):
    ...
    def test_current_version_matches_download_version(self, catalog_manager: CatalogManager, tmp_path: Path):
...
class TestBagPath:
    """Tests for DatasetBag.bag_path property — filesystem path to the bag root."""

    def test_path_returns_pathlib_path(self, catalog_manager: CatalogManager, tmp_path: Path):
        """bag.bag_path returns a pathlib.Path (not a string)."""
        catalog_manager.reset()
        ml, dataset_desc = catalog_manager.ensure_datasets(tmp_path / "source")
        dataset = dataset_desc.dataset
        ...
        assert isinstance(bag.bag_path, Path), f"bag.bag_path should be Path, got {type(bag.bag_path)}"

    def test_path_points_to_existing_directory(self, catalog_manager: CatalogManager, tmp_path: Path):
        """bag.bag_path points at a directory that exists on disk."""
        catalog_manager.reset()
        ml, dataset_desc = catalog_manager.ensure_datasets(tmp_path / "source")
        dataset = dataset_desc.dataset
        ...
        assert bag.bag_path.exists(), f"bag.bag_path does not exist on disk: {bag.bag_path}"
        assert bag.bag_path.is_dir(), f"bag.bag_path is not a directory: {bag.bag_path}"

    def test_path_contains_bag_structure(self, catalog_manager: CatalogManager, tmp_path: Path):
        """bag.bag_path directory contains the expected BDBag structure (data/, manifest)."""
        catalog_manager.reset()
        ml, dataset_desc = catalog_manager.ensure_datasets(tmp_path / "source")
        dataset = dataset_desc.dataset
        ...
        assert (bag.bag_path / "data").is_dir(), "bag should have a data/ subdirectory"
        manifests = list(bag.bag_path.glob("manifest-*.txt"))
        assert manifests, f"bag should have at least one manifest-*.txt file in {bag.bag_path}"


class TestBagDatasetProperties:
    """Tests for basic DatasetBag properties."""

    def test_dataset_rid_matches(self, catalog_manager: CatalogManager, tmp_path: Path):
    ...
    def test_dataset_types_match(self, catalog_manager: CatalogManager, tmp_path: Path):
    ...
    def test_description_preserved(self, catalog_manager: CatalogManager, tmp_path: Path):
...
class TestCacheDeletionAndRedownload:
    ...
    def test_delete_cache_then_redownload(self, catalog_manager: CatalogManager, tmp_path: Path):
...
class TestNonMaterializedBagAccess:
    ...
    def test_non_materialized_bag_can_list_members(self, catalog_manager: CatalogManager, tmp_path: Path):
    ...
    def test_non_materialized_bag_can_list_children(self, catalog_manager: CatalogManager, tmp_path: Path):
    ...
    def test_non_materialized_bag_can_denormalize(self, catalog_manager: CatalogManager, tmp_path: Path):
...
class TestBagMaterializeInPlace:
    ...
    def test_materialize_flips_metadata_only_to_materialized(self, catalog_manager: CatalogManager, tmp_path: Path):
        ...
        if BagCache._is_fully_materialized(bag.bag_path):
        ...
        assert BagCache._is_fully_materialized(bag.bag_path)
...
class TestDirectoryDatasetBagPath:
    """Tests for DatasetBag.path / .is_directory on directory datasets.

    Verifies that ``Directory_Dataset`` rows written by ``add_files`` export
    into the BDBag and are readable offline via ``DatasetBag.path`` and
    ``DatasetBag.is_directory``.
    """
    ...
    def test_dataset_bag_path_and_is_directory(
        ...
        - root bag has path == "." and is_directory is True
        - child bags have paths {"d1", "d2"} and is_directory is True
        - a non-directory dataset has path is None and is_directory is False
        ...
        assert root_dataset.path == ".", "live: root Dataset.path must be '.'"
        ...
        # materialization is not needed to test the path accessor).
        ...
        assert bag.path == ".", f"bag root .path should be '.', got {bag.path!r}"
        ...
        assert child_paths == {"d1", "d2"}, (
            f"bag child paths should be {{'d1','d2'}}, got {child_paths!r}"
        )
